...
describe('POST /upload-image/from-url', () => {
    let fetchSpy;
    ...
    it('抓取远程图标并转存到图床', async () => {
        fetchSpy
            ...
            .mockResolvedValueOnce(new Response(JSON.stringify({
                result: 'success',
                code: 200,
                url: `${PHOTO_HOST}/i/2026/09/21/Remote01.png`
            }), { status: 201 }));

        const res = await uploadImageApi.request(
            '/upload-image/from-url',
            fromUrlRequest('https://example.com/favicon.png'),
            createEnv()
        );
        ...
        // 抓取时伪造了 Referer，绕过防盗链
        const [, fetchInit] = fetchSpy.mock.calls[0];
        expect(fetchInit.headers.Referer).toBe('https://example.com');
    });

    it('远程返回的不是允许的图片类型时返回 415', async () => {
        fetchSpy.mockResolvedValueOnce(new Response('<html></html>', {
            status: 200,
        ...
        const res = await uploadImageApi.request(
            '/upload-image/from-url',
            fromUrlRequest('https://example.com/not-an-image'),
            createEnv()
        );
    ...
    it('拒绝非 http/https 地址', async () => {
        const res = await uploadImageApi.request(
            '/upload-image/from-url',
            fromUrlRequest('data:image/png;base64,iVBORw0KGgo='),
            createEnv()
        );
